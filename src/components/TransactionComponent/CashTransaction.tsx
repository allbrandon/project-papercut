...
const CashTransaction = ({ details }: { details: Transaction_details }) => {
  ...
  for (i = 0; i < details.item_list.length; i++) {
    if (details.item_list[i].discountamount) {
      sub_total =
        sub_total +
        (details.item_list[i].price + details.item_list[i].discountamount) *
          details.item_list[i].quantity;
      console.log(sub_total);
    } else {
      sub_total =
        sub_total + details.item_list[i].price * details.item_list[i].quantity;
      console.log(sub_total);
    }
  }
  ...
  if (details.cash < total) {
    return <div className="background">Data not valid</div>;
  } else {
  ...
  return (
    <div className="background">
      ...
      <div>
        ...
        <div>
          <ItemListComponent items={details.item_list} />
        </div>
        ...
        <div>
          <StoreDetail details={details.store_details} />
        </div>
      </div>
    </div>
  );
